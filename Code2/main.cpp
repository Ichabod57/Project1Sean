...
//solve for the eigenvalues and find the eigen vectors
	...
	/*
//tqli method to solve for e values and e vectors
	double *diag, *off_diag;
	diag = new double [step];
	off_diag = new double[step];
	tred2(SWE,step,diag,off_diag);
	tqli(diag, off_diag,step,EVector);
	*/
...
/*
//Unit test to check for normalizing EVectors
double NormCheck=0;
for (int i=0; i<step; i++){
	NormCheck += EVector[i][0]*EVector[i][0];}
...
EigenVectors.open("EigenVector_2ElectronCase_w5_length10_size300.txt");
