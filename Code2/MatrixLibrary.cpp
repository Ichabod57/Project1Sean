...
//fills a matrix with the 1's on the diagonal
...
//function to find the maximum off diagonal element for a symmetrix matrix
//Mat is the matrix, n is the size
//a and b are the positions in the matrix
double offdiagfind(double **Mat, int &a, int &b, int n){
	double max=0;
...
//find max off diagonal element for a non symmetrix matrix
//variable same as above

double offdiagfind2(double **Mat, int &a, int &b, int n){
	double max=0;
...
//Similar to Jacobi method higher up this is Seans personal version for symmetric matrices
//Use this with offdiagonal find to find a and b
//Rot is the matrix to be rotated
//EVec is the matrix with the eigen vectors
//a and b are where the sin's and cos's are in the rotation matrix
//step is just the size of the matrix

void JacobiRotSym(double ** Rot, double ** EVec, int &a, int &b, int step){
	double s, c, t, tau;
	double Rot_aa, Rot_bb, Rot_ia, Rot_ib, EVec_ia, EVec_ib;

	//Find what all of the trig functions are
	if (Rot[a][b] !=0){

		tau = (Rot[b][b] - Rot[a][a])/(2*Rot[a][b]);
	...
	Rot_aa = Rot[a][a];
	Rot_bb = Rot[b][b];
	Rot[a][a] = Rot_aa*c*c - 2*Rot[a][b]*c*s + Rot_bb*s*s;
	Rot[b][b] = Rot_bb*c*c + 2*Rot[a][b]*c*s + Rot_aa*s*s;
	Rot[a][b] = 0;
	Rot[b][a] = 0;
...
//end Jacobi rotation solver


//same as above but now solves for non symmetrices by using Jacobis mathod
...
//end non sym matrix solver

//Multiply two square matrices "a" and "b" of size n to get c
void MatMult(double **a, double **b, double **c, int n){
	...
	void VecMatMult(double **a, double *b, double *c, int n){
		for (int i=0; i<n; i++){
			for (int j=0; j<n; j++){
				c[i] += b[j]*a[j][i];}
		}
...
//end vector matrix multiply
	...
	void MatVecMult(double **a, double *b, double *c, int n){
		for (int i=0; i<n; i++){
			for (int j=0; j<n; j++){
				c[i] += b[j]*a[i][j];}
		}
